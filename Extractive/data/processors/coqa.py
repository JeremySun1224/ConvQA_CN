...
def coqa_convert_example_to_features(example, tokenizer, max_seq_length, doc_stride, max_query_length):
    ...
    for qa in example.question_text:
        query_tokens.extend(tokenizer.tokenize(qa))  # adding one by one

    cls_idx = CLS_SPAN
    if example.orig_answer_text == '是的':  # 后面也要对应改
        cls_idx = CLS_YES  # yes
    elif example.orig_answer_text == '不是':
        cls_idx = CLS_NO  # no
    elif example.orig_answer_text == '无法作答':
    ...
    for (i, token) in enumerate(example.doc_tokens):
        ...
        sub_tokens = tokenizer.tokenize(token)  # 相当于bertify()
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
        ...
        for i in range(doc_span.length):
            ...
            segment_ids.append(1)  # why append 1?
        tokens.append("[SEP]")
        segment_ids.append(1)
        # print(f'tokens: {tokens}')

        input_ids = tokenizer.convert_tokens_to_ids(tokens)  # get the input_ids
        ...
        if cls_idx >= CLS_SPAN:  # CLS_SPAN == 3
            # For training, if our document chunk does not contain an annotation, we throw it out, since there is nothing to predict.
            doc_start = doc_span.start
            doc_end = doc_span.start + doc_span.length - 1
            out_of_span = False
...
def coqa_convert_examples_to_features(examples, tokenizer, max_seq_length, doc_stride, max_query_length, is_training, threads=1):
    ...
    if not is_training:
        ...
        # dataset = TensorDataset(all_input_ids, all_tokentype_ids, all_input_mask, all_start_positions, all_end_positions, all_rational_mask, all_cls_idx)
        # batch is here
        dataset = TensorDataset(all_input_ids, all_tokentype_ids, all_input_mask, all_start_positions, all_end_positions, all_cls_idx)  # dataset[-2] == all_rational_mask
...
class CoqaProcessor(DataProcessor):
    train_file = "ConvQA_CN_trainset.json"
    dev_file = "ConvQA_CN_devset.json"
    ...
    def process(self, parsed_text):
        ...
        word_idx = 0
        for sent in parsed_text.sents:  # 分句
    ...
    def get_raw_context_offsets(self, words, raw_text):
        ...
        for token in words:
            while p < len(raw_text) and re.match('\s', raw_text[p]):
                p += 1
            # if raw_text[p:p + len(token)] != token:  # 对于中文，由于都是单个字符，所以需重新考虑
    ...
    def get_examples(self, data_dir, history_len, filename=None, threads=1):
        ...
        Args:
            ...
            filename: None by default, specify this if the training file has a different name than the original one which is `coqa-train-v1.0.json`.
        """
        if data_dir is None:
            data_dir = ""
    ...
    def _create_examples(self, input_data, history_len):
        ...
        examples = []
        datum = input_data  # 原始数据的一个单元
        context_str = datum['Passage']
        context_str = context_str.replace(u"\u0005", "")
        # 新格式数据的一个单元
        _datum = {
            'context': context_str,
            'source': datum['Source'],
        ...
        # 获取每个词在原文本中的开始和结束字符位置，如'I am fine.'中fine的位置是字符5到9(不含9)。
        ...
        for i in range(len(datum['Questions'])):
            ...
            yes_words = ['是的', '是', '对的', '有']
            no_words = ['不是', '没有', '不能']
            unknown_words = ['原文', '无法', '作答']

            nlp_answer = nlp(self.pre_proc(answer['input_text']))
            annotated_answer = self.process(nlp_answer)
            annotated_answer_words = annotated_answer['word']  # 分词结果

            if self.is_inter(yes_words, annotated_answer_words):
                _qas['raw_answer'] = '是的'
            if self.is_inter(no_words, annotated_answer_words):
                _qas['raw_answer'] = '不是'
            if self.is_inter(unknown_words, annotated_answer_words):
                _qas['raw_answer'] = '无法作答'

            # 如果正确答案出现在start和end之间，则记录该位置；否则找一段与正确答案重合最多的位置，这将会导致generative类型的answer范围有误
            if _qas['answer_type'] == 'extractive':
                start = answer['answer_start'][0]  # 多个开始位置默认首次出现的位置为正确答案的开始位置
                end = start + len(_qas['answer']) - 1
                _qas['answer_span_start'] = start
                _qas['answer_span_end'] = end
            else:
                generative_index = _datum['context'].find(_qas['answer'])
                if generative_index != -1:
                    start = generative_index
                    end = start + len(_qas['answer']) - 1
                else:
                    find_substring_idx = self.find_lcsubstr(s1=_datum['context'], s2=_qas['answer'])
                    if find_substring_idx:
                        longest_substring = find_substring_idx[0]
                        longest_substring_len = find_substring_idx[1]
                        start = _datum['context'].find(longest_substring)
                        end = start + longest_substring_len - 1
                    else:
                        start = 0
                        end = 0
                ...
                # start = _datum['context'].find(_qas['answer'])  # 那么对于generative类型的answer的开始位置都默认为-1
            ...
            example = CoqaExample(
                qas_id=str(_datum['id']) + ' ' + str(_qas['turn_id']),  # 这里的str(_datum['id'])将会导致evaluate.py的部分不统一，需要对应修改。
...
if __name__ == '__main__':
    ...
    # # 先要把数据经过CoqaProcessor()进行预处理后，才交给Tokenizer进行特征转换
    examples = processor.get_examples(data_dir='../raw_data_cn', history_len=2)
    # for example in examples:
    #     print(example)
    #     feature = coqa_convert_example_to_features(
    #         example=example,
    #         tokenizer=tokenizer,
    #         max_seq_length=512,
    #         doc_stride=128,
    ...
    #     exit(0)
    print(examples)
    # coqa_convert_example_to_features(example, tokenizer, max_seq_length, doc_stride, max_query_length):
    # features = coqa_convert_example_to_features(
    #     ex
    # )
    ...
    #     threads=1
    # )
    # examples = coqa_proprecess.get_examples(data_dir='../raw_data_cn', history_len=2)
    # print(examples)
    # exit(0)
    # with open('../raw_data_cn/ConvQA_CN_trainset.json', mode='r', encoding='utf-8') as reader:
    #     input_data = json.load(reader)['data']
    #     # print(input_data)
    #     child_story, news, childcare, history = 0, 0, 0, 0
    #     for datum in input_data:
    #         if datum['Source'] == '儿童故事':
    #             child_story += len(datum['Questions'])
    #         if datum['Source'] == '热点新闻':
    #             news += len(datum['Questions'])
    #         if datum['Source'] == '育儿知识':
    #             childcare += len(datum['Questions'])
    #         if datum['Source'] == '历史':
    #             history += len(datum['Questions'])
    #     print(f'各领域总问答轮数    儿童故事：{child_story}, 热点新闻: {news}, 育儿知识: {childcare}, 历史: {history}')
