...
def compute_predictions_logits(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(tqdm(all_examples, desc="Writing predictions")):
        ...
        for pred in prelim_predictions:
            ...
            if pred.cls_idx == CLS_SPAN:
                ...
                tok_text = tokenizer.convert_tokens_to_string(tok_tokens)  # converts a sequence of token ids in a single string.

                # Clean whitespace
                tok_text = tok_text.strip()
                tok_text = " ".join(tok_text.split())  # 这里应该是去除预测答案间空格的地方
                orig_text = " ".join(orig_tokens)

                final_text = get_final_text(tok_text, orig_text, do_lower_case, verbose_logging)  # project the tokenized prediction back to the original text.
        ...
        if len(nbest) < 1:
            nbest.append(_NbestPrediction(text='无法作答', score=-float('inf'), cls_idx=CLS_UNK))
        ...
        for i, entry in enumerate(nbest):
            ...
            # output["text"] = entry.text.encode('utf-8').decode('unicode_escape')  # 这样还是乱码
...
def confirm_preds(nbest_json):
    ...
    subs = [
        'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'twelve', 'true', 'false'
    ]
...
def coqa_evaluate(dev_file, output_prediction_file):
    ...
    with open(output_prediction_file, encoding='utf-8') as f:
