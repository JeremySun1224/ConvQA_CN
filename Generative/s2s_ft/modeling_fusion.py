...
class BertForSequenceToSequence(BertPreTrainedForSeq2SeqModel):
    ...
    def forward(self, source_ids, target_ids, pseudo_ids, num_source_tokens, num_target_tokens, start_positions, end_positions, ans_choice, target_span_ids=None):
        ...
        target_ids_fake = torch.zeros_like(target_ids)
        pseudo_ids_fake = torch.zeros_like(pseudo_ids)
        num_target_tokens_fake = torch.zeros_like(num_target_tokens)

        input_ids_ex = torch.cat((source_ids, target_ids_fake, pseudo_ids_fake), dim=1)
        token_type_ids_ex = torch.cat(
            (torch.ones_like(source_ids) * self.source_type_id,
             torch.ones_like(target_ids_fake) * self.target_type_id,
             torch.ones_like(pseudo_ids_fake) * self.target_type_id), dim=1)
             
        source_mask_ex, source_position_ids_ex = \
            self.create_mask_and_position_ids(num_source_tokens, source_len)
        target_mask_ex, target_position_ids_ex = \
            self.create_mask_and_position_ids(num_target_tokens_fake, target_len, offset=num_source_tokens)

        position_ids_ex = torch.cat((source_position_ids_ex, target_position_ids_ex, target_position_ids_ex), dim=1)
        if target_span_ids is None:
            target_span_ids = target_position_ids_ex
        attention_mask_ex = self.create_attention_mask(source_mask_ex, target_mask_ex, source_position_ids_ex, target_span_ids)

        outputs_ex = self.bert(
            input_ids_ex, attention_mask=attention_mask_ex, token_type_ids=token_type_ids_ex,
            position_ids=position_ids_ex, split_lengths=split_lengths)

        sequence_output_ex = outputs_ex[0]
        extract_sequence_output = sequence_output_ex[:,:source_len, ] 
