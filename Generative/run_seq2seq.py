...
def get_model_and_tokenizer(args):
    ...
    model = BertForSequenceToSequence.from_pretrained(
        args.model_name_or_path + "pytorch_model.bin", config=config, model_type=args.model_type,
        ...
        for step, batch in enumerate(train_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and \
                    ...
                    # torch.save(model_to_save.state_dict(), os.path.join(args.output_dir, "model.{}.bin".format(global_step)))
